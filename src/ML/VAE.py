...
class VariationalAutoencoder(object):
    ...
    def cleanup(self):
        # Avoid TF profiler __del__ bug at interpreter shutdown
        self.profiler = None
...
def train_whole(sess,vae, input_data, learning_rate=0.0001, batch_size=100, training_epochs=10, display_step=200, vae_mode=True, vae_mode_modalities=True):
    ...
    for epoch in tqdm((range(training_epochs))):
        ...
        if epoch % display_step == 0:
            ...
            print("Epoch: %04d / %04d, Cost= %04f, Recon= %04f, Latent= %04f, Interval= %.3fs" % \
                (epoch,training_epochs,avg_cost, avg_recon, avg_latent, elapsed_s))
        
                       
    ### Save the trained model
    param_id= 1
    model_dir = os.path.join(filepath, "outputs", "ml", "models", "Models1")
    os.makedirs(model_dir, exist_ok=True)
    save_path = vae.saver.save(vae.sess, os.path.join(model_dir, f"model_{param_id}.ckpt"))
