...
parser.add_argument(
    "--plot-filename",
    required=True,
    help="PDF filename written under MachineLearning/plots",
)
args = parser.parse_args()

filepath = "/home/franklin/Research"
results_dir = f"{filepath}/MachineLearning/results"
os.makedirs(results_dir, exist_ok=True)
plots_dir = f"{filepath}/MachineLearning/plots"
os.makedirs(plots_dir, exist_ok=True)

SYN_IMU_DIR = f"{filepath}/Dataset/vIMU/Augmented/syndata_half_radial"
EXP_IMU_DIR = f"{filepath}/Dataset/vIMU/Experimental/expdata_half_radial"
SYN_IMU_PREFIX = "syndata_half_radial"
EXP_IMU_PREFIX = "Rajapogal_FullBody_calibrated"
SYN_KINEMATICS_PATH = f"{filepath}/Dataset/Motion/Augmented/syndata.mot"
EXP_KINEMATICS_PATH = f"{filepath}/Dataset/Motion/Experimental/expdata.mot"
...
def read_imu_tables(imu_dir, imu_prefix):
    quaternion_data = pl.read_csv(
        f"{imu_dir}/{imu_prefix}_orientations.sto",
        separator="\t",
        skip_rows=4,
    ).with_columns(pl.col("time").cast(pl.Float64))

    velocity_data = pl.read_csv(
        f"{imu_dir}/{imu_prefix}_angular_velocity.sto",
        separator="\t",
        skip_rows=4,
    ).with_columns(pl.col("time").cast(pl.Float64))

    acceleration_data = pl.read_csv(
        f"{imu_dir}/{imu_prefix}_linear_accelerations.sto",
        separator="\t",
        skip_rows=4,
    ).with_columns(pl.col("time").cast(pl.Float64))
...
with tf.Graph().as_default():
    with tf.Session() as sess:
        ...
        new_saver.restore(sess, f"{filepath}/MachineLearning/models/Models1/model_1.ckpt")
...
pdf_path = os.path.join(plots_dir, args.plot_filename)
