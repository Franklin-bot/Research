...
class VirtualIMULocationTests(unittest.TestCase):
    def test_builds_five_non_rotated_locations_for_each_segment(self):
        ...
        self.assertEqual(names["up"], [
            "humerus_r_imu9_0deg",
            "humerus_l_imu9_0deg",
            "ulna_r_imu9_0deg",
            "ulna_l_imu9_0deg",
        ])
        self.assertEqual(names["down"], [
    ...
    def test_masks_kinematics_and_all_but_retained_location_for_both_times(self):
        ...
        for i, col in enumerate(columns[len(KINEMATICS_COLS) :], start=len(KINEMATICS_COLS)):
            if col.startswith("seg_imu9_0deg_"):
                np.testing.assert_array_equal(masked[:, i], data[:, i])
            else:
                self.assertTrue(np.all(masked[:, i] == -2.0), col)
